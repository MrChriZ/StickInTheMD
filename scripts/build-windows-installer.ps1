# Build StuckInTheMD Release and compile the Windows installer (Inno Setup 6).
param(
...
$ExePath = Join-Path $SourceDir "stuckinthemd.exe"
...
if (-not $SkipBuild) {
    ...
    Write-Host "Building stuckinthemd ($Configuration)..."
    & cmake --build $BuildDir --config $Configuration --target stuckinthemd
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
}
