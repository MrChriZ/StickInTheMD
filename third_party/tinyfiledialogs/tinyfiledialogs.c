...
#ifdef _WIN32
void * tinyfd_winOwner = NULL;
static HWND tinyfd_winOwnerHwnd(void) {
	return tinyfd_winOwner ? (HWND)tinyfd_winOwner : NULL;
}
static HINSTANCE tinyfd_winInstance(void) {
	...
	wchar_t const * const aSingleFilterDescription) /* NULL or "image files" */
{
	...
	ofn.hwndOwner = tinyfd_winOwnerHwnd();
	ofn.hInstance = tinyfd_winInstance();
	...
	wchar_t const * const aDefaultPath) /* NULL or "" */
{
	...
	bInfo.hwndOwner = tinyfd_winOwnerHwnd();
	bInfo.pidlRoot = NULL;
	...
	unsigned char aoResultRGB[3]) /* { 0 , 0 , 0 } */
{
	...
	cc.hwndOwner = tinyfd_winOwnerHwnd();
	cc.hInstance = NULL;
